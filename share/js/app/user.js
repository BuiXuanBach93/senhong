searchUser = function() {
    $('#fromdate1').datepicker({pickTime: false, format: "dd-mm-yyyy" });
    $('#todate1').datepicker({pickTime: false, format: "dd-mm-yyyy" });     
    $('#userTable').DataTable( {
        destroy: true,
        processing: false,
        serverSide: true,
        ...
        columnDefs: [ 
            {
                ...
                render: function (data, type, full, meta) { 
                    if(document.getElementById('parent').value == 'listUser')
                        return data;
                    else 
                        return full[6];
                }
        ...
        ajax: {
            ...
            data: function ( d ) {
                ...
                d.mobile = $('#mobile1').val();
                d.home = $('#home1').val();
                d.usertype = $('#usertype1').val();      
                d.fromdate = $('#fromdate1').val();
                d.todate = $('#todate1').val();
                d.sortasc = $('#sortAsc').val();
...
validateClient = function() {
    ...
    if(document.getElementById('username').value == null || document.getElementById('username').value.trim().length == 0) {
        ...
        return false;
    } else if(document.getElementById('mobile').value.trim().length < 10) {
        showFieldError('mobile','Số điện thoại phải nhiều hơn 9 ký tự');
        return false;
    }
    else if(document.getElementById('birthday').value == null || document.getElementById('birthday').value.trim().length == 0) {
        showFieldError('birthday','Hãy nhập ngày sinh');
        return false;
    }
    else if(document.getElementById('idnumber').value == null || document.getElementById('idnumber').value.trim().length == 0) {
        showFieldError('idnumber','Hãy nhập chứng minh thư');
        return false;
    }
    ...
    else if(document.getElementById('address').value == null || document.getElementById('address').value.trim().length == 0) {
        showFieldError('address','Hãy nhập địa chỉ');
        return false;
    }
    else if(document.getElementById('home').value == null || document.getElementById('home').value.trim().length == 0) {
        showFieldError('home','Hãy nhập số nhà');
        return false;
    }
    return true;
}
...
loadViewUser = function(viewUserId) {
    $.ajax({ 
        ...
        success: function(data, textStatus, jqXHR) {
            ...
            document.getElementById('viewDiv').innerHTML = data.page;
            document.getElementById('lblTitle').innerHTML = 'THÔNG TIN NGƯỜI DÙNG';
            ...
            if(data.user.user_id) {
                ...
                if(data.user.user_type == '3') {
                    document.getElementById('listplan').href = '/listplan?maidid=' + data.user.user_id;
                    document.getElementById('listtask').href = '/listtask?maidid=' + data.user.user_id;
                    document.getElementById('listwork').href = '/listwork?userid=' + data.user.user_id;
                }
                if(data.user.user_type == '4') {
                    document.getElementById('listplan1').href = '/listplan?customerid=' + data.user.user_id;
                    //document.getElementById('listtask1').href = '/listtask?customerid=' + data.user.user_id;
                    document.getElementById('listorder').href = '/listorder?userid=' + data.user.user_id;
                }
            }
            ...
            if(data.user.birthday) document.getElementById('lblbirthday').innerHTML =  convertToDDMMYYY(data.user.birthday);
...
convertToDDMMYYY = function(dateStr){
...
getMyInfo = function(viewUserId) {
    ...
    $.ajax({ 
        ...
        cache: false, //fix loop IE
        success: function(data, textStatus, jqXHR) {
            $('#birthday').datepicker({pickTime: false, format: "dd-MM-yy" });
...
changeSort = function() {
    if(document.getElementById('sortAsc').value == '0') {
        document.getElementById('sortAsc').value = '1';
        document.getElementById('sortIcon').className = 'fa fa-caret-up';
    } else {
        document.getElementById('sortAsc').value = '0';
        document.getElementById('sortIcon').className = 'fa fa-caret-down';
    }
}