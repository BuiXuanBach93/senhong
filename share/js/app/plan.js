searchPlan = function() {
    ...
    $('#orderTable').DataTable( {
        ...
        ajax: {
            ...
            url: '/searchplan',
            data: function ( d ) {
                d.maidid = $('#maidid1').val();
                d.customerid = $('#customerid1').val();
                d.ordercode = $('#ordercode1').val();
                d.customer = $('#customer1').val();
                d.maid = $('#maid1').val();
...
viewPlan = function(id) {
    $.ajax({ 
        ...
        success: function(data, textStatus, jqXHR) {
            ...
            document.getElementById('lblTitle').innerHTML = 'NỘI DUNG LỊCH';
            document.getElementById('parent').value = parentPage;
            document.getElementById('planId').value = data.plan.plan_id;
            document.getElementById('orderId').value = data.plan.order_id;
...
editPlan = function() {
    ...
    if(obj.innerHTML == '<span class="fa fa-pencil"></span> Sửa') {
        ...
        var cboStatus = document.getElementById('status');
        for(var i = 0; i < cboStatus.options.length; i++) {
            if(cboStatus.options[i].innerHTML == document.getElementById('lblStatus').innerHTML) {
                cboStatus.value = cboStatus.options[i].value;
                break;
            }
        }
...
updatePlan = function() {
    $.ajax({ 
        ...
        success: function(data, textStatus, jqXHR) {
            ...
            document.getElementById('lblMaidName').innerHTML = cboMaidId.options[cboMaidId.selectedIndex].innerHTML;
            var cboStatus = document.getElementById('status');
            document.getElementById('lblStatus').innerHTML = cboStatus.options[cboStatus.selectedIndex].innerHTML;
            document.getElementById('lblStart').innerHTML = document.getElementById('start').value;
            document.getElementById('lblEnd').innerHTML = document.getElementById('end').value;
            editPlan();
...
sortStart = function() {
    if(document.getElementById('sortStartIcon').className == 'fa fa-caret-down') {
        document.getElementById('sortType').value = '1';
        document.getElementById('sortStartIcon').className = 'fa fa-caret-up';
    } else {
        document.getElementById('sortType').value = '0';
        document.getElementById('sortStartIcon').className = 'fa fa-caret-down';
    }
}

sortEnd = function() {
    if(document.getElementById('sortEndIcon').className == 'fa fa-caret-down') {
        document.getElementById('sortType').value = '3';
        document.getElementById('sortEndIcon').className = 'fa fa-caret-up';
    } else {
        document.getElementById('sortType').value = '2';
        document.getElementById('sortEndIcon').className = 'fa fa-caret-down';
    }
}